...
<div class="ua-card">
    ...
    <form action="<?= base_url('admin/user-access/update/' . esc($user['id'])) ?>" method="post">
        <?= csrf_field() ?>
        <div class="ua-grid">
            <div>
                <div class="ua-label">First Name</div>
                <input type="text" name="first_name" class="ua-input" value="<?= esc($user['first_name'] ?? '') ?>">
            </div>
            <div>
                <div class="ua-label">Middle Name</div>
                <input type="text" name="middle_name" class="ua-input" value="<?= esc($user['middle_name'] ?? '') ?>">
            </div>
            <div>
                <div class="ua-label">Last Name</div>
                <input type="text" name="last_name" class="ua-input" value="<?= esc($user['last_name'] ?? '') ?>">
            </div>
            <div style="grid-column: span 2;">
                <div class="ua-label">Address</div>
                <input type="text" name="address" class="ua-input" value="<?= esc($user['address'] ?? '') ?>" placeholder="Street, City, Province">
            </div>
            <div>
                <div class="ua-label">Username</div>
                <input type="text" name="username" class="ua-input" value="<?= esc($user['username'] ?? '') ?>">
            </div>
            <div>
                <div class="ua-label">Email</div>
                <input type="email" name="email" class="ua-input" value="<?= esc($user['email']) ?>" required>
            </div>
            <div>
                <div class="ua-label">Password <span style="color: #6b7280; font-size: 11px;">(leave blank to keep current)</span></div>
                <input type="password" name="password" class="ua-input">
            </div>
            <div>
        ...
        <div style="margin-top:12px;display:flex;justify-content:flex-end;gap:8px;">
            ...
            <button type="submit" class="ua-input" style="width:auto;border-radius:999px;border:none;background:#1d4ed8;color:#ffffff;font-weight:600;cursor:pointer;">Update Account</button>
        </div>
    </form>
</div>
